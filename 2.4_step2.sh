...
for pool in "$POOL_RS" "$POOL_CLAY" "$POOL_LRC" "$META_POOL"; do
  sudo cephadm shell -- ceph osd pool delete "$pool" "$pool" \
    --yes-i-really-really-mean-it || true
done

# delete profiles
for profile in "$PROFILE_RS" "$PROFILE_CLAY" "$PROFILE_LRC"; do
...
########################################
echo ">>> [3/9] 配置 RS EC Pool (k=6, m=4) -> $POOL_RS"
########################################
...
########################################
echo ">>> [4/9] 配置 CLAY EC Pool (k=6, m=3, d=8) -> $POOL_CLAY"
########################################
...
########################################
echo ">>> [5/9] 配置 LRC EC Pool (k=6, m=2, l=4) -> $POOL_LRC"
########################################
...
########################################
echo ">>> [6/9] 启用 RBD-on-EC（必须）：allow_ec_overwrites + application rbd"
########################################
...
########################################
echo ">>> [7/9] 创建 RBD images：3张 EC-backed images (metadata=$META_POOL, data-pool=EC pool)"
########################################
...
########################################
echo ">>> [8/9] 宿主机：map -> mkfs.xfs -> mount (RS + CLAY + LRC)"
########################################
...
sudo cephadm shell -- ceph osd pool ls detail | egrep "($META_POOL|$POOL_RS|$POOL_CLAY|$POOL_LRC)" || true
...
df -h "$MNT_RS" "$MNT_CLAY" "$MNT_LRC" || true