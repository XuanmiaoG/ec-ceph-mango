...
echo ">>> [2/11] 安装系统基础依赖[cite: 8]..."
sudo apt update
sudo apt install -y python3 python3-pip git ansible
ansible --version

echo ">>> [3/11] 下载 Ceph-Ansible 代码库[cite: 8]..."
...
echo ">>> [4/11] 安装 Python 依赖和 Ansible Collections[cite: 8]..."
...
echo ">>> [6/11] 生成 Inventory 文件 (12节点/10 OSD)[cite: 9, 10]..."
...
echo ">>> [7/11] 配置全局变量 (all.yml)[cite: 11, 12]..."
...
echo ">>> [8/11] 配置角色变量 (mons, osds, mgrs)[cite: 12]..."
...
echo ">>> [10/11] 最终预检 (Ping & Disk)[cite: 14]..."
# 检查连通性
ansible -i inventory/hosts all -m ping
# 检查 OSD 磁盘是否存在
ansible -i inventory/hosts osds -a "lsblk /dev/sdb"

echo ">>> [11/11] 开始部署[cite: 15]..."
